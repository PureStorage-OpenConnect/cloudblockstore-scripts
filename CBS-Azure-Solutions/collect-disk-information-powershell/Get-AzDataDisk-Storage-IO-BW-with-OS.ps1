...
.CHANGELOG
    - 1.3 Included OS Disk information in the report
...
.OUTPUTS
        ...
        .\Get-AzDataDisk-Storage-IO-BW-with-OS.ps1
...
$virtualMachines | ForEach-Object -Parallel {
    ...
    $osDisks | ForEach-Object -Parallel {
        $osDisk = $_
        ...
        $diskSize = Get-AzDisk -ResourceGroupName $vm.ResourceGroupName -DiskName $osDisk.Name | Select-Object -ExpandProperty DiskSizeGB
        $diskIops = Get-AzDisk -ResourceGroupName $vm.ResourceGroupName -DiskName $osDisk.Name | Select-Object -ExpandProperty DiskIOPSReadWrite
        $diskBandwidth = Get-AzDisk -ResourceGroupName $vm.ResourceGroupName -DiskName $osDisk.Name | Select-Object -ExpandProperty DiskMBpsReadWrite
        $diskSKU = Get-AzDisk -ResourceGroupName $vm.ResourceGroupName -DiskName $osDisk.Name | Select-Object -ExpandProperty Sku | Select-Object -ExpandProperty Name
        ...
        $metrics = Get-AzMetric `
            -ResourceId "/subscriptions/$subscriptionId/resourceGroups/$resourceGroup/providers/Microsoft.Compute/disks/$osDiskName" `
            -StartTime $startTime `
            -TimeGrain $TimeGrain `
        ...
        $existingDisk = $osDiskInfo | Where-Object { $_.DataDiskName -eq $osDisk.Name }
        if ($existingDisk) {
            ...
            $outputObject = New-Object -TypeName PSObject -Property $osDiskHashtable
            $dataDiskInfoTSDictionary[$osDisk.Name] = $outputObject;
        }
    }
